    ...
    def spawn_zombie(self, channel):
        if self.is_connected():
            ...
            if random.random() < self.boss_chance:
                self.zombies[channel][zombie_id] = {"spawn_time": time.time(), "health": self.boss_health, "is_boss": True}
                self.connection.privmsg(channel, f"A boss zombie (ID: {zombie_id}) appears! It takes {self.boss_health} bullets to kill. Type '!shoot {zombie_id}' to shoot it!")
            else:
                self.zombies[channel][zombie_id] = {"spawn_time": time.time(), "health": 1, "is_boss": False}
                self.connection.privmsg(channel, f"A wild zombie (ID: {zombie_id}) appears! Type '!shoot {zombie_id}' to kill it!")
    ...
    def handle_shooting(self, c, e, parts):
        ...
        try:
            ...
            if random.random() < self.explosion_chance:
                ...
                c.privmsg(channel, f"{user}, due to the gun explosion you've lost 2 kills. Your current score is now {self.scores[user]}")
                self.save_scores()
            else:
                channel_lower = channel.lower()
                if zombie_id in self.zombies.get(channel_lower, {}):
                    ...
                    if "health" in zombie:
                        if zombie["is_boss"]:
                            if zombie["health"] > 1:
                                zombie["health"] -= 1
                                c.privmsg(channel, f"Nice shot, {user}! You hit boss zombie {zombie_id}. It now has {zombie['health']} health left.")
                            else:
                            ...
                            del self.zombies[channel_lower][zombie_id]
                            self.scores[user] = self.scores.get(user, 0) + 1
                            c.privmsg(channel, f"Nice shot, {user}! You killed zombie {zombie_id} in {shot_time:.2f} seconds! Your current score is now {self.scores[user]}.")
                            self.save_scores()
                    else:
